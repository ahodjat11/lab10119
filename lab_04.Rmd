...
#arcos.wv.mingo.54059.itemized -> mingo

###lets make sure that worked

#print(mingo)

#looks like it did! cool.
...
###oh wow, they are all lowercase
###nothing else really jumps out, though
...
```{r}
#mingo_working <- mingo %>%
  #mutate(state_distribution = if (reporter_state = WV) in_state_distribution else out_of_state_distrubution)
...
###it appears that Miami-Luken/Tug Valley/Actavis was the most significant pipeline.
###i used the 'view' function here because for some reason the button to run the chunks disappeared
...
#view(mingo %>%
  #group_by(reporter_name, reporter_dea_no, reporter_address1) %>%
  #summarise(pills_per_pharmacy = sum(dosage_unit)) %>%
  #arrange(desc(pills_per_pharmacy)))
  
###see chunk above for why I'm using the view function
###pretty much the same code as above — dea no and address were included as distinguishing factors, pill no. i left in just out of curiosity
...
```{r}
#First of all, thank you for not making me write the code for this.

#I would imagine that there's some sort of library that allows us to do this. Failing that, however, my best guess would be to nest the if_then function within the grouping function, somehow. I remember in the last lab, we were able to return text results with a certain text string in them using the $ as an anchor. I'm sure there must be something within the tidyverse — or janitor — that lets us do that, right?

#Failing THAT, it's time to use the text editor. 
